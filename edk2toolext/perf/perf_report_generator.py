...
INF_GUID_REGEX = r"\s*FILE_GUID\s*\=\s*([0-9a-fA-F]{8,8}-[0-9a-fA-F]{4,4}-[0-9a-fA-F]{4,4}-[0-9a-fA-F]{4,4}-[0-9a-fA-F]{12,12})\s*"
    ...
    for root, _, files in os.walk(input_path):
        for name in files:
            ...
            if name.lower().endswith(".dec"):
                ...
                _parse_file(
                    file_path, (INF_FILE_REGEX, INF_GUID_REGEX), guid_dict, is_inf=True
                )
            elif name.lower().endswith(".fdf"):
                _parse_file(
                    file_path, (FDF_FILE_REGEX, FDF_GUID_REGEX), guid_dict, is_fdf=True
                )
...
def main() -> int:
    ...
    if options.output_debug_log:
        if len(options.output_debug_log) < 2:
            ...
            file_logger = logging.FileHandler(
                filename=options.output_debug_log, mode="w"
            )
            if options.debug:
    ...
    logging.info(
        f"Log Started: {datetime.datetime.strftime(datetime.datetime.now(), '%A, %B %d, %Y %I:%M%p')}"
    )
    ...
    timing_list, messages, unrecognized_records = parse_fpdt_records(
        options.input_xml_file, text_log, guids
    )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-i",
        "--input_xml_file",
        help="Path to the input XML file with raw FPDT data",
        default=None,
    )
    parser.add_argument(
        "-s",
        "--src_tree_root",
        help="Root of UEFI Code tree to parse for GUIDs",
        nargs="*",
        default=None,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-l",
        "--output_debug_log",
        dest="output_debug_log",
        help="Log all debug and error output to file",
        default=None,
    )
...
def parse_fpdt_records(input_xml_file: str, text_log: str, guids: dict) -> tuple:
    ...
    for record in fbpt:
        if record.tag == "FirmwareBasicBootPerformanceEvent":
            ...
            messages.append(
                ("Error", f"Unrecognized record found with tag {record.tag}")
            )
...
def _process_dynamic_string_event(
    ...
    progress_id_string_minus_one = (
        f"0x{int(record.find('ProgressID').attrib['Value'], 16) - 1:04X}"
    )
    ...
    # Can be Function, Crossmodule, or Inmodule begin
        ...
        if label in start_records_dict:
            ...
            messages.append(
                ("Error", f"Start record with label {label} already in dict!")
            )
        else:
    ...
    # Can be Function, Crossmodule, or Inmodule end
        ...
        if label in start_records_dict:
            ...
            length = (
                float(record.find("Timestamp").attrib["ValueInMilliseconds"])
                - start_time
            )
            if length < 0:
            ...
            logging.error(
                f"Error! Can't find a start event in the dict for label {label}!"
            )
            messages.append(
                ("Error", f"Can't find a start event in the dict for label {label}!")
            )
    else:
    ...
    # Can be load_image, binding_support, binding_stop end
        ...
        if label in start_records_dict:
            ...
            if event_type == "LoadImage":
                ...
                qword = record.find("Qword").attrib["Value"]
                string_value = (
                    controller_handle_dict[qword]
                    if (qword in controller_handle_dict)
                    else "Unknown Device"
                )
...
def _process_guid_qword_string_event(
    ...
    if progress_id_string_minus_one not in PROGRESS_ID_LABEL_DICT:
        logging.critical(
            f"Error! Unrecognized progress ID {progress_id_string_minus_one}"
        )
        messages.append(
            ("Error", f"Unrecognized progress ID {progress_id_string_minus_one}")
        )
        unrecognized_records += 1
        return id_counter, unrecognized_records
    ...
    if "554E-4B4E484E444C" in guid:
        guid_string = (
            f"HANDLE-NOT-FOUND-{hashlib.sha256(guid.encode('utf-8')).hexdigest()[0:4]}"
        )
    else:
        guid_string = (
            guids[guid]
    ...
    if label in start_records_dict:
        ...
        del start_records_dict[label]
        length = (
            float(record.find("Timestamp").attrib["ValueInMilliseconds"]) - start_time
        )
        if length < 0:
            logging.error("Error! Cannot have negative time!")
...
def _check_unmatched_start_records(
    start_records_dict: dict[str, float], messages: list[tuple[str, str]]
) -> None:
...
def _write_text_output(
    timing_list: list[tuple[int, str, str, str, str, float, float]], text_log: "TextIO"
) -> None:
...
def write_html_report(
    ...
    for line in template.readlines():
        if "%TO_BE_FILLED_IN_BY_PYTHON_SCRIPT%" in line:
            ...
            html_report.write(
                f'"DateCollected": "{date_collected_xml.attrib["Value"]}",'
            )
            html_report.write(f'"ReportGenVersion": "{REPORT_GENERATOR_VER}",')
            html_report.write(
                f'"FpdtParserVersion": "{fpdt_parser_version_xml.attrib["Value"]}",'
            )
            ...
            for message in messages:
                ...
                first = False
                html_report.write(
                    f'{{"Type": "{message[0]}", "Message": "{message[1]}"}}'
                )
            html_report.write("]}};")
        else:
